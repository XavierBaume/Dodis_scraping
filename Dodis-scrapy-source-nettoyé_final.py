...
class DodisSpider(scrapy.Spider):
    ...
    start_urls = [f"https://dodis.ch/{str(i).zfill(5)}" for i in range(60000, 70000)]  # Ajustez la plage
    ...
    def parse(self, response):
        ...
        document = {
            ...
            # Créer un publication_year afin d'obtenir une valeur numérique supplémentaire?
            'publication_year': response.xpath("//meta[@itemprop='datePublished']/@content").re_first(r"\d{4}"),
        ...
        tags = {
            ...
            # Important à garder ? Modifier ? Quid liste ? 
            'tags_multilang': {
        ...
        structured_data = {
         ...
         # Calcul du total des éléments dans `other_persons_mentioned`, `other_organizations_mentioned`, et `document_tags`
        total_elements = {
            'total_other_persons_mentioned': len(persons['other_persons_mentioned']),
            'total_other_organizations_mentioned': len(organizations['other_organizations_mentioned']),
            'total_document_tags': len(tags['document_tags']),
        }

        structured_data['totals'] = total_elements
